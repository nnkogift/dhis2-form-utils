...
DHIS2 metadata types were hand-written in `@nnkogift/dhis2-form-utils-metadata`. This duplicated the
official OpenAPI schema, drifted from the API, and covered only a subset of `ValueType` values.

## Decision

1. Add `@dhis2/api-types` v43 as a dev dependency of `@nnkogift/dhis2-form-utils-metadata` and
...
- Runtime rule-evaluation shapes (`FieldState`, `FieldStateMap`) in `@nnkogift/dhis2-form-utils-rules`.
- Normalised consumer types (`FieldConfig`) in `@nnkogift/dhis2-form-utils-hooks` that deliberately differ
  from API shapes.
