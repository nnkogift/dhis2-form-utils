...
## Confirmed decisions (supersede open questions below)

These were resolved during planning (see `/Users/gnnko/.claude/plans/handoff-implement-unsupported-sequential-thacker.md` for full rationale) and override the corresponding sections/open questions further down this doc:

- **C1 (coordinate wire format):** `"[lng,lat]"` JSON array string — confirmed against DHIS2 tracker API docs. Closes open question #2 in `use-field-control-plan.md` §7.
- **C2 (GEOJSON):** gets its own `widgetKind: 'geojson'`, split off from `coordinate` — not the "keep alias" or "detect point-only" options originally listed.
- **Map UI (new scope beyond this doc's original plan):** both `coordinate` and `geojson` widgets get a real MapLibre GL JS map (marker picker / draw editor via `terra-draw`), not just number inputs. Housed in a new shared `packages/map` package (`@nnkogift/dhis2-form-utils-map`) so the 3 UI adapters don't each bundle their own copy.
- **O1 (org-unit data source):** hybrid — optional `OrgUnitPickerProvider` + default `useDataQuery` fetch. Closes open question #4.
- **F2 (delete on clear):** skip — matches this doc's original recommendation.
- **Outstanding:** GEOJSON's exact DE-level value shape (bare `Geometry` vs. full `Feature`) was not conclusively verified during planning — flagged for QA against a live DHIS2 instance before that widget's validation is final.
...
- [x] `coordinateValue` parse/join helpers + unit tests
- [x] `buildFieldSchema` cases: `COORDINATE`, `ORGANISATION_UNIT`, `FILE_RESOURCE`, `IMAGE` (plus `GEOJSON`)
- [x] Align `buildTeaFieldSchema` (OU already present; add the rest)
- [x] Decide `GEOJSON` mapping; adjust `widgetKind.ts` + tests
- [x] Optional: `useFileResourceUpload`, org-unit picker context types exported from hooks

### UI adapters (each of dhis2-ui / mantine / mui)

- [x] `D2CoordinateField`, `D2GeoJsonField`, `D2OrgUnitField`, `D2FileField`, `D2ImageField`
- [x] Export from `widgets/index.ts`
- [x] Point `WIDGET_BY_KIND` entries away from `D2UnsupportedField`
- [x] Keep `unsupported` → `D2UnsupportedField`
- [x] `fallow-ignore-next-line` **kinds only** (no prose after `--` — that creates fake stale kinds)
- [x] Prefer file-level `// fallow-ignore-file code-duplication` on mirrored `D2Field.tsx` if clone groups fire (also applied to `MapFields.tsx`, which triggered a real clone group)

### Storybook

- [x] Expand `TIER1_WIDGET_KINDS` / stubs in `apps/storybook/fixtures/fieldMetadata.ts` (via `NETWORK_WIDGET_KINDS`)
- [x] Stories: `Coordinate`, `GeoJson`, `OrgUnit`, `FileUpload`, `ImageUpload` per adapter
- [x] Interactions in `fieldStoryInteractions.ts` (picker-friendly queries; avoid brittle `input[type=…]` only)
- [x] MSW handlers for OU tree + fileResources (+ map tiles)
- [x] Remove `orgUnit` from `STUB_WIDGET_KINDS` once interactive

### Docs

- [x] Update valueType → widget table in `docs/use-field-control-plan.md` §7 with real components + `[lng,lat]`
- [x] Close open questions #2 and #4 (or record decisions here and link)
- [x] Touch `apps/docs` reference pages (`reference/hooks`, new `reference/map`) when widgets ship
- [x] Tracker how-to: clarify DE/TEA `ORGANISATION_UNIT` vs enrollment `orgUnit`
