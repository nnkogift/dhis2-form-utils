...
| 2   | **`COORDINATE` serialisation** — open; recommended default `[lng,lat]` JSON string. Tracked in [`unsupported-fields-handoff.md`](./unsupported-fields-handoff.md).                                                  | `CoordinateField` and tracker payload builder   |
...
| 4   | **`ORGANISATION_UNIT` data fetching** — open; recommended hybrid provider + default query. Tracked in [`unsupported-fields-handoff.md`](./unsupported-fields-handoff.md).                                           | `OrgUnitField` in all adapters                  |
